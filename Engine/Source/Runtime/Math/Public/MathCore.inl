...
inline void FVector3::Flatten() noexcept
{
    this->y = 0.f;
}
...
inline FAngles FQuat::ToEulerAngles() const noexcept
{
    FAngles r;
    ...
    r.pitch = std::atan2( sinr_cosp, cosr_cosp );

    // pitch (y-axis rotation)
    float sinp = 2 * (w * y - z * x);
    if (std::abs( sinp ) >= 1)
        r.yaw = std::copysign( XM_PI / 2, sinp ); // use 90 degrees if out of range
    else
        r.yaw = std::asin( sinp );
    ...
    r.roll = std::atan2( siny_cosp, cosy_cosp );
