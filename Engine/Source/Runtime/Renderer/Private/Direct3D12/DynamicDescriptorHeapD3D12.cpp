...
//
// DynamicDescriptorHeapD3D12 Implementation
//

CriticalSection DynamicDescriptorHeapD3D12::sm_Mutex;
std::vector<Microsoft::WRL::ComPtr<ID3D12DescriptorHeap>> DynamicDescriptorHeapD3D12::sm_DescriptorHeapPool[2];
std::queue<std::pair<uint64, ID3D12DescriptorHeap*>> DynamicDescriptorHeapD3D12::sm_RetiredDescriptorHeaps[2];
std::queue<ID3D12DescriptorHeap*> DynamicDescriptorHeapD3D12::sm_AvailableDescriptorHeaps[2];

ID3D12DescriptorHeap* DynamicDescriptorHeapD3D12::RequestDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE HeapType)
{
    sm_Mutex.Enter();

    uint32 idx = HeapType == D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER ? 1 : 0;
    CommandManagerD3D12* CmdManager = DCast<CommandManagerD3D12*>(GCommandManager);
...
void DynamicDescriptorHeapD3D12::DiscardDescriptorHeaps(D3D12_DESCRIPTOR_HEAP_TYPE HeapType, uint64 FenceValue, const std::vector<ID3D12DescriptorHeap*>& UsedHeaps)
{
    uint32 idx = HeapType == D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER ? 1 : 0;
    sm_Mutex.Enter();
...
void DynamicDescriptorHeapD3D12::RetireCurrentHeap(void)
{
...
void DynamicDescriptorHeapD3D12::RetireUsedHeaps(uint64 fenceValue)
{
    DiscardDescriptorHeaps(m_DescriptorType, fenceValue, m_RetiredHeaps);
    m_RetiredHeaps.clear();
}

DynamicDescriptorHeapD3D12::DynamicDescriptorHeapD3D12(CommandContextD3D12& OwningContext, D3D12_DESCRIPTOR_HEAP_TYPE HeapType)
...
DynamicDescriptorHeapD3D12::~DynamicDescriptorHeapD3D12()
{
}

void DynamicDescriptorHeapD3D12::CleanupUsedHeaps(uint64 fenceValue)
{
    RetireCurrentHeap();
...
inline ID3D12DescriptorHeap* DynamicDescriptorHeapD3D12::GetHeapPointer()
{
...
uint32 DynamicDescriptorHeapD3D12::DescriptorHandleCache::ComputeStagedSize()
{
...
void DynamicDescriptorHeapD3D12::DescriptorHandleCache::CopyAndBindStaleTables(
...
void DynamicDescriptorHeapD3D12::CopyAndBindStagedTables(DescriptorHandleCache& HandleCache, ID3D12GraphicsCommandList* CmdList,
    void (STDMETHODCALLTYPE ID3D12GraphicsCommandList::* SetFunc)(UINT, D3D12_GPU_DESCRIPTOR_HANDLE))
{
...
void DynamicDescriptorHeapD3D12::UnbindAllValid(void)
{
    m_GraphicsHandleCache.UnbindAllValid();
    m_ComputeHandleCache.UnbindAllValid();
}

D3D12_GPU_DESCRIPTOR_HANDLE DynamicDescriptorHeapD3D12::UploadDirect(D3D12_CPU_DESCRIPTOR_HANDLE Handle)
{
    ID3D12Device* pID3D12Device = RCast<ID3D12Device*>(GDevice->GetNativeDevice());
    HE_ASSERT(pID3D12Device != NULL);
...
void DynamicDescriptorHeapD3D12::DescriptorHandleCache::UnbindAllValid()
{
    m_StaleRootParamsBitMap = 0;
...
void DynamicDescriptorHeapD3D12::DescriptorHandleCache::StageDescriptorHandles(uint32 RootIndex, uint32 Offset, uint32 NumHandles, const D3D12_CPU_DESCRIPTOR_HANDLE Handles[])
{
...
void DynamicDescriptorHeapD3D12::DescriptorHandleCache::ParseRootSignature(D3D12_DESCRIPTOR_HEAP_TYPE Type, const IRootSignature& RootSig)
{
