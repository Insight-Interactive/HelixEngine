// Custom Helix Engine implementation built on top of imgui dx12 backend.
...
static void ImGui_ImplDX12_CreateFontsTexture()
{
	...
	// Upload texture to graphics system
	{
		...
		D3D12_CPU_DESCRIPTOR_HANDLE CpuDescriptorHandle = AllocateDescriptor( bd->pd3dDevice, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV );
		D3D12_CPU_DESCRIPTOR_HANDLE ImGuiTextureCpuHandle{ bd->Handle.GetCpuPtr() };
		...
		bd->pd3dDevice->CreateShaderResourceView( pTexture, &srvDesc, CpuDescriptorHandle );
		...
		bd->pd3dDevice->CopyDescriptors( 1, &ImGuiTextureCpuHandle, &DestCount, DestCount, SourceTextures, SourceCounts, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV );
	...
	// [Solution 3] IDE/msbuild: edit imconfig.h and add '#define ImTextureID ImU64' (prefer solution 2 to create your own config file!)
	// [Solution 4] command-line: add '/D ImTextureID=ImU64' to your cl.exe command-line (this is what we do in the example_win32_direct12/build_win32.bat file)
	static_assert(sizeof(ImTextureID) >= sizeof( bd->Handle.GetGpuPtr() ), "Can't pack descriptor handle into TexID, 32-bit not supported yet.");
	io.Fonts->SetTexID((ImTextureID)bd->Handle.GetGpuPtr() );
}
...
bool ImGui_ImplDX12_Init(ID3D12Device* device, int num_frames_in_flight, DXGI_FORMAT rtv_format)
{
	...
	io.BackendRendererName = "HelixImGuiImplD3D12";
...
static void ImGui_ImplDX12_RenderWindow(ImGuiViewport* viewport, void*)
{
	...
	cmd_list->SetGraphicsRootDescriptorTable( 1, TextureGpuHandle );
